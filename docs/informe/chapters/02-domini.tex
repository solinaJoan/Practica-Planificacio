...
\begin{itemize}
    ...
    \item \texttt{(servida ?r - reserva)}: indica que la reserva \texttt{?r} ha estat assignada exitosament a una habitació. Aquest predicat es necessari per expressar l'objectiu del nivell bàsic, on totes les reserves han d'estar servides.
...
\subsection{Funcions Numèriques}
...
\begin{itemize}
    \item \texttt{(reserves-assignades)}: comptador utilitzat a partir de l'extensió 1 per penalitzar les reserves no assignades. S'incrementa en 5 (extensió 1) o en 1 (extensions 2, 3 i 4) cada vegada que es descarta una reserva. El valor diferent a l'extensió 1 és per donar-li més pes en la funció objectiu.
